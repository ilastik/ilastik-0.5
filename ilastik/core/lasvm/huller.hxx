...
public:
    ...
    void XPxk_and_XNxk(const std::vector<T>& row,const int index,T& XPxk,T &XNxk) const;
...
template<class Kernel>
void hullerSVM<Kernel>::XPxk_and_XNxk(const std::vector<T>& row,const int index,T& XPxk,T &XNxk) const
{
    XPxk=0.0;
    XNxk=0.0;
